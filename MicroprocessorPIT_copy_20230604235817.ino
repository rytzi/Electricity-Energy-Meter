...
#define FIREBASE_PROJECT_ID "electricity-energy-meter-56b20"
FirebaseData fbdo;
FirebaseAuth auth;
...
void writeToFirebase() {
  if (Firebase.ready() && (millis() - dataMillis > 6 || dataMillis == 0))
    {
        dataMillis = millis();

        FirebaseJson content;

        // String documentPath = "ESP32" + String(count);

        content.set("fields/VRMS/doubleValue", emon.Vrms);
        content.set("fields/IRMS/doubleValue", emon.Irms);
        content.set("fields/Power/doubleValue", emon.apparentPower);
        content.set("fields/KWH/doubleValue", kWh);

        String documentPath = "/ESP32/QQRJkXDTmP1eqKOkKrAZ";

        // String documentPath = "projects/";
        // documentPath += FIREBASE_PROJECT_ID;
        // documentPath += "/databases/(default)/documents/ESP32/QQRJkXDTmP1eqKOkKrAZ";
        count++;
    if(Firebase.Firestore.patchDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), content.raw(), "VRMS,IRMS,Power,KWH")){
      Serial.printf("ok\n");
      return;
    }else{
      Serial.println(fbdo.errorReason());
        ...
        // if (Firebase.Firestore.createDocument(&fbdo, , "electricity-energy-meter-56b20", documentPath.c_str(), content.raw()))
        //     Serial.printf("ok\n%s\n\n");
        // else
        //     Serial.println(fbdo.errorReason());
    }
